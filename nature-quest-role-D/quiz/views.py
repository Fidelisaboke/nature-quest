...
from .serializers import (
    QuizSerializer, QuizDetailSerializer, QuizResultSerializer, QuestionSerializer, 
    QuizAttemptSerializer, QuizSubmissionSerializer, QuestionBankSerializer,
    QuizGenerationRequestSerializer, QuizMetricsSerializer, QuizStatsSerializer
)
...
class QuizViewSet(viewsets.ModelViewSet):
    ...
    def submit(self, request, pk=None):
        ...
        result = quiz_service.submit_quiz(quiz.id, submission_serializer.validated_data['responses'])
        
        if result['success']:
            # If passed (80%+), award points via userprogress app
            if result['passed']:
            ...
            return Response({
                'result': result,
                'quiz_details': result_serializer.data
            })
        else:
            return Response(result, status=status.HTTP_400_BAD_REQUEST)
    
    def _award_quiz_completion_points(self, user, quiz, score):
        """Award points to user for completing quiz (via userprogress app API)"""
        try:
            ...
            # Make internal API call to userprogress app
            ...
            if response.status_code == 200:
                logger.info(f"Awarded {points} points to {user.username} for quiz completion")
            else:
