...
from userprogress.models import UserProfile, Badge, Level, UserBadge, PointsTransaction
from quiz.models import Quiz, Question, QuizAttempt, QuestionResponse, QuestionBank
from challengeverification.models import Challenge, ChallengeAttempt, PhotoAnalysis, LocationVerification
...
class Command(BaseCommand):
    ...
    def create_test_data(self):
        ...
        if not users:
            self.stdout.write(self.style.ERROR('❌ No users found. Run create_sample_users first'))
            return
        ...
        self.stdout.write(f'   - Challenge Attempts: {ChallengeAttempt.objects.count()}')
        self.stdout.write(f'   - Points Transactions: {PointsTransaction.objects.count()}')
    ...
    def create_quizzes(self):
        ...
        if created:
            self.stdout.write(f'  ✅ Created quiz: Nature Knowledge Quiz for {quiz.user.username}')
            ...
            questions_data = [
                {
                    ...
                    'tech_stack': 'Nature Knowledge',
                    'difficulty': 'medium',
                    'options': ['Black Bear', 'Grizzly Bear', 'Polar Bear', 'Kodiak Bear'],
                ...
                },
                {
                    ...
                    'tech_stack': 'Nature Knowledge',
                    'difficulty': 'medium',
                    'options': ['Osmosis', 'Transpiration', 'Xylem transport', 'Photosynthesis'],
    ...
    def create_quiz_attempts(self, users, quizzes):
        ...
        for quiz in quizzes:
            ...
            if created:
                ...
                self.stdout.write(f'  ✅ {quiz.user.username} completed quiz for challenge {quiz.challenge_id}')
            else:
                self.stdout.write(f'  ⚠️ Quiz attempt already exists for {quiz.user.username}')
        ...
        for i, user in enumerate(users[:3]):
            for challenge in challenges[:2]:  # Use first 2 challenges
                ...
                attempt, created = ChallengeAttempt.objects.get_or_create(
                    ...
                    defaults={
                        'status': 'verified',
                        'submitted_photo': ContentFile(img_file.read(), name=f'test_photo_{user.id}_{challenge.id}.jpg'),
                        ...
                        'verification_details': {'camera': 'test', 'location': 'verified'},
                ...
                if created:
                    ...
                    self.stdout.write(f'  ⚠️ Attempt already exists: {user.username} -> {challenge.title}')
    ...
    def create_points_transactions(self, users):
        ...
        transactions = [
            {'points': 100, 'transaction_type': 'earned', 'description': 'Completed Forest Wildlife Quiz with 90% score'},
            {'points': 75, 'transaction_type': 'earned', 'description': 'Successfully verified location at Yellowstone'},
            {'points': 200, 'transaction_type': 'earned', 'description': 'Won weekly nature photography contest'},
            {'points': 150, 'transaction_type': 'earned', 'description': 'Completed Mountain Peak Photography Challenge'}
        ]
