...
class ProgressService:
    ...
    @staticmethod
    def update_user_progress(user_id, points_to_add, transaction_type, description, 
                           challenge_id=None, quiz_id=None, increment_challenges=False, 
        ...
        try:
            with transaction.atomic():
                ...
                achievements = ProgressService._check_achievements(user, profile, old_points)
                
                logger.info(f"Updated progress for user {user.username}: +{points_to_add} points")
    ...
    def _check_achievements(user, profile, old_points):
        ...
        badges_available = Badge.objects.filter(
        ...
        for badge in badges_available:
            ...
            if badge.is_special and not ProgressService._check_special_badge_eligibility(user, profile):
                continue
                
            UserBadge.objects.create(
                user=user,
                badge=badge,
                points_when_earned=current_points
            )
        ...
        if new_level and (not old_level or new_level.level_number > old_level.level_number):
    ...
    def _check_special_badge_eligibility(user, profile):
        ...
        return (
            regular_badges_count >= 12 and
            profile.current_level and profile.current_level.level_number >= 10 and
            profile.challenges_completed >= 20
        )
    ...
    def _get_next_badge(profile):
        ...
        next_badge = Badge.objects.filter(
            points_required__gt=profile.total_points
        ).exclude(
