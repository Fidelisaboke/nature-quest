...
from challengeverification.models import Challenge
import random
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options['clear']:
            ...
            self.stdout.write(
                self.style.WARNING('Cleared all existing challenges')
            )
        ...
        challenge_templates = [
            ...
            },
            {
                ...
                'points_reward': 300,
                'required_elements': ['mountain peak', 'sunrise or sunset', 'clear sky'],
            ...
            },
            {
                ...
                'difficulty_level': 'intermediate',
                'points_reward': 150,
                'required_elements': ['lake reflection', 'mountain or tree reflections'],
        ...
        for i in range(count):
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"Error creating challenge {template['title']}: {str(e)}")
                )
        ...
        self.stdout.write(f"Total active challenges: {Challenge.objects.filter(is_active=True).count()}")
        ...
        self.stdout.write('User progress: GET /api/challenge-verification/challenges/my-progress/')
        self.stdout.write('Admin stats: GET /api/challenge-verification/admin/challenges/statistics/')
