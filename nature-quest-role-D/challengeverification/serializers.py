...
from .models import Challenge, ChallengeAttempt, PhotoAnalysis, LocationVerification, VerificationMetrics, FraudDetection
...
class ChallengeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Challenge
        fields = ['id', 'title', 'description', 'difficulty_level', 'location_type',
                 'location_name', 'target_latitude', 'target_longitude',
                 'required_elements', 'special_instructions', 'points_reward', 
                 'verification_radius', 'is_active', 'completion_count', 'created_at']
...
class ChallengeListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Challenge
        fields = ['id', 'title', 'difficulty_level', 'location_type', 'points_reward',
                 'completion_count']
...
class PhotoAnalysisSerializer(serializers.ModelSerializer):
    class Meta:
        model = PhotoAnalysis
        fields = ['exif_data', 'has_location_data', 'photo_latitude', 'photo_longitude',
                 'timestamp', 'detected_objects', 'confidence_scores', 
                 'image_quality_score', 'authenticity_score', 'nature_elements',
                 'element_confidence', 'created_at']


class LocationVerificationSerializer(serializers.ModelSerializer):
    class Meta:
        model = LocationVerification
        fields = ['foursquare_places', 'closest_match', 'distance_to_match',
                 'location_type_match', 'is_valid_coordinate', 'country_code',
                 'region', 'nearby_landmarks', 'verification_confidence', 'created_at']


class FraudDetectionSerializer(serializers.ModelSerializer):
    class Meta:
        model = FraudDetection
        fields = ['risk_level', 'duplicate_image_detected', 'suspicious_location',
                 'metadata_inconsistencies', 'rapid_submissions', 'risk_factors',
                 'confidence_score', 'requires_manual_review', 'created_at']
...
class ChallengeAttemptSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'username', 'challenge_details', 'status', 'submitted_photo',
                 'submitted_latitude', 'submitted_longitude', 'submission_notes',
                 'photo_verified', 'location_verified', 'verification_details',
                 'points_earned', 'bonus_points', 'total_points',
                 'created_at', 'verified_at', 'photo_analysis', 
                 'location_verification', 'fraud_detection']
        read_only_fields = ['status', 'photo_verified', 'location_verified',
                          'verification_details', 'points_earned', 'bonus_points',
                          'verified_at']
...
class VerificationMetricsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['challenge_title', 'total_attempts', 'successful_verifications',
                 'failed_verifications', 'success_rate', 'average_verification_time',
                 'photo_failures', 'location_failures', 'quality_failures',
                 'last_updated']
