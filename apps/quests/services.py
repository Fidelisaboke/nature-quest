...
class PhotoVerificationService:
    ...
    def __init__(self):
        ...
        self.quality_threshold = float(getattr(dj_settings, "PHOTO_QUALITY_THRESHOLD", 0.6))
        self.authenticity_threshold = float(getattr(dj_settings, "PHOTO_AUTHENTICITY_THRESHOLD", 0.7))
    ...
    def analyze_photo(self, image_file, challenge: Challenge) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_results["verification_passed"] = (
                analysis_results["quality_score"] >= self.quality_threshold
                and analysis_results["authenticity_score"]
                >= self.authenticity_threshold
                and analysis_results["timestamp_valid"]
                and analysis_results["has_required_elements"]
            )
    ...
    def _check_required_elements(
        self, detected_elements: List[Dict], required_elements: List[str]
    ) -> bool:
        ...
        try:
            ...
            for required in required_elements:
                found = any(
                    required.lower() in detected.lower()
                    or detected.lower() in required.lower()
...
class LocationVerificationService:
    ...
    def __init__(self):
        from django.conf import settings as dj_settings
        self.api_key = getattr(dj_settings, "LOCATION_VERIFICATION_API_KEY", getattr(dj_settings, "FOURSQUARE_API_KEY", ""))
        self.base_url = getattr(dj_settings, "LOCATION_VERIFICATION_BASE_URL", getattr(dj_settings, "FOURSQUARE_API_URL", "https://api.foursquare.com/v3/places"))
        self.timeout = int(getattr(dj_settings, "LOCATION_VERIFICATION_TIMEOUT", 15))

    def verify_location(
        self, latitude: float, longitude: float, challenge: Challenge
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if foursquare_result.get("success"):
                verification_results["location_type_match"] = foursquare_result.get(
                    "type_match", False
                )
                verification_results["distance_to_target"] = foursquare_result.get(
                    "distance", float("inf")
                )

                # Calculate confidence based on distance and type match
                distance = verification_results["distance_to_target"]
                # Use challenge.verification_radius if set, else default from settings
                verification_radius = getattr(challenge, "verification_radius", None)
                if not verification_radius:
                    from django.conf import settings as dj_settings
                    verification_radius = int(getattr(dj_settings, "DEFAULT_VERIFICATION_RADIUS", 500))
                if distance <= verification_radius:
                    base_confidence = 1.0 - (distance / verification_radius)
                    if verification_results["location_type_match"]:
                        verification_results["confidence_score"] = min(
                            base_confidence + 0.2, 1.0
                        )
                    else:
                ...
                verification_results["verification_passed"] = (
                    ...
                    and verification_results["confidence_score"] >= 0.6
                )
    ...
    def _verify_with_foursquare(
        ...
        try:
            ...
            category = self._map_location_type_to_foursquare_category(
            ...
            response = requests.get(
                f"{self.base_url}/nearby", headers=headers, params=params, timeout=self.timeout
            )
    ...
    def _check_location_type_match(
        self, expected_type: str, place_categories: List[str]
    ) -> bool:
...
class FraudDetectionService:
    ...
    def _check_rapid_submissions(self, user: User) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"is_suspicious": is_suspicious, "recent_count": recent_submissions}
    ...
    def _check_location_plausibility(self, attempt: ChallengeAttempt) -> Dict[str, Any]:
        ...
        try:
            ...
            last_attempt = (
                ChallengeAttempt.objects.filter(
                    user=attempt.user, created_at__lt=attempt.created_at
                )
                .order_by("-created_at")
                .first()
            )
            ...
            if last_attempt:
                time_diff = (
                    attempt.created_at - last_attempt.created_at
                ).total_seconds() / 3600  # hours
                distance = self._calculate_distance(
                    last_attempt.submitted_latitude,
                    last_attempt.submitted_longitude,
