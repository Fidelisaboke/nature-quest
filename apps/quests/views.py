            ...
            return Response(
                {
                    ...
                    "completed_challenges": len(
                        [p for p in progress_data.values() if p["is_completed"]]
                    ),
                }
            )
...
class ChallengeAttemptViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ChallengeAttempt.objects.filter(user=self.request.user).order_by(
            "-created_at"
        )
        ...
        try:
            with transaction.atomic():
                ...
                challenge = get_object_or_404(
                    Challenge, id=serializer.validated_data["challenge_id"]
                )
                ...
                attempt = ChallengeAttempt.objects.create(
                    ...
                    submitted_longitude=serializer.validated_data[
                        "submitted_longitude"
                    ],
                    submitted_description=serializer.validated_data.get(
    ...
    def _process_verification(self, attempt: ChallengeAttempt) -> dict:
        ...
        try:
            ...
            photo_analysis = photo_service.analyze_photo(
                attempt.submitted_photo, attempt.challenge
            )
                ...
                analysis_notes=json.dumps(
                    {
                        ...
                        "has_required_elements": photo_analysis.get(
            ...
            location_verification_obj = LocationVerification.objects.create(
                attempt=attempt,
                is_within_radius=location_verification.get(
                    "verification_passed", False
                ),
                distance_to_target=location_verification.get("distance_to_target", 0.0),
                foursquare_data=location_verification.get(
                    "foursquare_verification", {}
                ),
                verification_confidence=location_verification.get(
                    "confidence_score", 0.0
                ),
                verification_passed=location_verification.get(
                    "verification_passed", False
                ),
            )
            ...
            fraud_detection_obj = FraudDetection.objects.create(
                ...
                requires_manual_review=fraud_analysis.get(
            ...
            if photo_passed and location_passed and fraud_ok:
                ...
                # Award points/badges (call to progress API)
    ...
    def _award_completion_rewards(self, attempt: ChallengeAttempt):
        ...
        try:
            # This would call the progress API to award points
            # For now, we'll just log it
            points_awarded = attempt.challenge.points_reward
            logger.info(
                f"Awarded {points_awarded} points to user {attempt.user.id} for challenge {attempt.challenge.id}"
            )

            # In a real implementation, make API call to progress app:
            # from django.conf import settings as dj_settings
            # requests.post(f"{dj_settings.PROGRESS_API_URL}/award-points/", {
...
class VerificationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def photo_analysis(self, request):
        ...
        try:
            ...
            attempt = get_object_or_404(
                ChallengeAttempt, id=attempt_id, user=request.user
            )
    ...
    def location_verification(self, request):
        ...
        try:
            ...
            location_verification = LocationVerification.objects.filter(
                attempt=attempt
            ).first()
    ...
    def metrics(self, request):
        ...
        try:
            ...
            success_rate = (
                (verified_attempts / total_attempts * 100) if total_attempts > 0 else 0
            )
            ...
            try:
                ...
                logger.warning(
                    f"VerificationMetrics query failed: {str(metrics_error)}"
                )
                metrics = {
            ...
            return Response(
                {
                    ...
                    "performance_metrics": {
                        ...
                        "avg_location_verification_time": metrics.get(
                            "avg_location_time"
                        )
                        or 0,
                    },
    ...
    def statistics(self, request):
        ...
        try:
            ...
            for challenge in challenges:
                ...
                total_attempts = ChallengeAttempt.objects.filter(
                    challenge=challenge
                ).count()
                successful_attempts = ChallengeAttempt.objects.filter(
                    challenge=challenge, verification_status="verified"
                ).count()
...
class AdminFraudDetectionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if requires_review is not None:
            queryset = queryset.filter(
                requires_manual_review=requires_review.lower() == "true"
            )
    ...
    def _award_completion_rewards(self, attempt):
        ...
        try:
            points_awarded = attempt.challenge.points_reward
            logger.info(
                f"Awarded {points_awarded} points to user {attempt.user.id} for manually approved challenge {attempt.challenge.id}"
            )
        except Exception as e:
            logger.error(f"Error awarding completion rewards: {str(e)}")
