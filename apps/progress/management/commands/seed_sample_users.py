...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--with-superuser", action="store_true", help="Create an admin superuser"
        )
    ...
    def handle(self, *args, **options):
        if options["clear"]:
            ...
            self.stdout.write(self.style.WARNING("Cleared all non-superuser accounts"))
        ...
        if options["with_superuser"]:
            try:
                if not User.objects.filter(username="admin").exists():
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Created superuser: admin (token: {token.key})"
                        )
                    )
                else:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"Superuser already exists: admin (token: {token.key})"
                        )
                    )
        ...
        if created_users:
            ...
            self.stdout.write(
                f'   curl -H "Authorization: Token {token}" http://127.0.0.1:8000/api/challenge-verification/challenges/'
            )
            self.stdout.write(
                f'   curl -H "Authorization: Token {token}" http://127.0.0.1:8000/api/progress/profile/'
            )
            self.stdout.write(
                f'   curl -H "Authorization: Token {token}" http://127.0.0.1:8000/api/quiz/quizzes/'
            )
        ...
        try:
            ...
            self.stdout.write(
                "Total tokens: Unable to count (Token app may not be installed)"
            )
