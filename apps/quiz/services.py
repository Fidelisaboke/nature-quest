...
class HuggingFaceQuizGenerator:
    ...
    def __init__(self):
        ...
        self.model = getattr(settings, "HUGGINGFACE_MODEL", "microsoft/DialoGPT-large")
        self.timeout = int(getattr(settings, "HUGGINGFACE_TIMEOUT", 30))
        self.max_retries = int(getattr(settings, "HUGGINGFACE_MAX_RETRIES", 3))

    def generate_questions(
        self, tech_stack: str, difficulty: str, count: int = 5
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            logger.info(
                f"Generated {len(questions)} questions for {tech_stack} ({difficulty})"
            )
            return questions
    ...
    def _generate_single_question(
        self, tech_stack: str, difficulty: str, question_type: str
    ) -> Optional[Dict]:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        return self._parse_generated_text(
                            result, tech_stack, difficulty, question_type
                        )
                    else:
                        logger.warning(
                            f"HF API error: {response.status_code} - {response.text} (attempt {attempt+1})"
                        )
                except Exception as e:
                    logger.error(f"Error calling Hugging Face API (attempt {attempt+1}): {str(e)}")
            return None
...
"""
    ...
    def _parse_generated_text(
        self, result: Dict, tech_stack: str, difficulty: str, question_type: str
    ) -> Optional[Dict]:
    ...
    def _validate_question_data(
        self, data: Dict, tech_stack: str, difficulty: str, question_type: str
    ) -> Optional[Dict]:
        ...
        try:
            ...
            correct_answers = [
                idx for idx in correct_answers if 0 <= idx < len(options)
            ]
    ...
    def _get_fallback_questions(
        self, tech_stack: str, difficulty: str, count: int
    ) -> List[Dict]:
        ...
        while len(questions) < count:
            questions.append(
                self._create_template_question(tech_stack, difficulty, len(questions))
            )

        return questions[:count]

    def _create_template_question(
        self, tech_stack: str, difficulty: str, index: int
    ) -> Dict:
...
class QuizService:
    ...
    def create_quiz_for_user(
        self, user_id: int, challenge_id: int, tech_stack: str, difficulty: str
    ) -> Optional[Quiz]:
        ...
        try:
            with transaction.atomic():
                ...
                if existing_quiz:
                    logger.info(
                        f"Quiz already exists for user {user_id}, challenge {challenge_id}"
                    )
                ...
                if not question_data_list:
                    logger.error(
                        f"Failed to generate questions for {tech_stack} ({difficulty})"
                    )
                    return None
    ...
    def submit_quiz(self, quiz_id: int, responses: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(
                    f"Quiz {quiz_id} submitted: {score:.1f}% ({'PASSED' if passed else 'FAILED'})"
                )
    ...
    def _update_metrics(self, quiz: Quiz):
        ...
        try:
            metrics, created = QuizMetrics.objects.get_or_create(
                ...
                defaults={"total_quizzes": 0, "total_passes": 0, "average_score": 0.0},
            )
        ...
        try:
            ...
            if completed_quizzes.exists():
                ...
                stats["overall_pass_rate"] = (
                    passed_quizzes.count() / completed_quizzes.count()
                ) * 100

                # Find favorite tech stack
                tech_stacks = completed_quizzes.values_list("tech_stack", flat=True)
                if tech_stacks:
                    stats["favorite_tech_stack"] = max(
                        set(tech_stacks), key=list(tech_stacks).count
                    )
